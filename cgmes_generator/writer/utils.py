...
def py_imports(meta: ClassMeta) -> Tuple[List[str], str | None]:
    ...
    if meta.name == "TopologicalNode" and "TopologyProfile" in ".".join(meta.pkg_parts):
        path = _rel_mod(
            meta.pkg_parts, meta.pkg_parts[:-1] + ("Core",), "IdentifiedObject"
        )
        imps.add(f"from {path} import IdentifiedObject")
    if meta.parent and meta.parent_pkg:
        ...
        imps.add(
            f"from {path} import {meta.parent}"
            + (f" as {alias}" if alias != meta.parent else "")
        )
        parent_alias = alias
    for a in meta.attrs.values():
        base = a.type_
